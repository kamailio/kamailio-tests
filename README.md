...
`docker` subfolder. These are:
  ...
  * `docker/Dockerfile.debian13` - container build with Debian 13.x (Trixie) deploying Kamailio installed
  from source code. The directory with Kamailio source code is copied from local disk into the
  container
  * `docker/Dockerfile.debian12` - container build with Debian 12.x (Bookworm) deploying Kamailio installed
  from source code. The directory with Kamailio source code is copied from local disk into the
  container
  * `docker/Dockerfile.debian11` - container build with Debian 11.x (Bullseye) deploying Kamailio installed
  from source code. The directory with Kamailio source code is copied from local disk into the
  container
